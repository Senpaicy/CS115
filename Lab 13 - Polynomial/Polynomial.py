...
# Favorite flavor of dorito : JACKED [Ranch Dipped Hot Wings]
# *****
...
class Term:
    ...
    def __eq__(self, other): # myTerm == other
        ...
        if(self.coef == other.coef)&(self.exp==other.exp):
            return True
        return False
...
# exponent is strictly decreasing
    ...
    def __mul__(self, otherPoly): # mulPoly = myPoly * otherPoly
        ...
        partial = LinkedPolynomial([])
        total = LinkedPolynomial([])
        for i in range(len(self.polyList)):
            x = self.polyList[i]
            for j in range(len(otherPoly.polyList)):
                g = otherPoly.polyList[j]
                partial.addTerm(Term(x.coef * g.coef,x.exp + g.exp))  
        return partial
        
              
    def __eq__(self, other): # myPoly == other
        '''Checks if 2 polynomials are equal. Note no polynomials will have
        0-coefficient terms. Hint: If two LP are equal, then they will be identical in every way '''
        if (self.polyList==other.polyList):
            return True
        return False
